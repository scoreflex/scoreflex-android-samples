...
package com.scoreflex.samples.realtime.RealtimeConnectionManagement;
...
import com.scoreflex.realtime.RealtimeMap;
import com.scoreflex.realtime.SessionInitializedListener;
import com.scoreflex.realtime.ConnectionListener;
...
public class MainActivity extends Activity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    findViewById(R.id.connect_button).setOnClickListener(
      new View.OnClickListener() {
        @Override
        public void onClick(View view) {
          String txt = (String)((Button)view).getText();
  ...
  private void initializeSDK() {
    ...
    ((Button)findViewById(R.id.connect_button)).setEnabled(false);
    ((Button)findViewById(R.id.profile_button)).setEnabled(false);
    ((Button)findViewById(R.id.connect_button)).setText(R.string.connect);
  ...
  private void startRealtimeSession() {
      ...
      Session.initialize(new SessionInitializedListener() {
        public void onSuccess() {
          ...
          ((Button)findViewById(R.id.connect_button)).setEnabled(true);
  ...
  private void connectRealtimeSession() {
    ((Button)findViewById(R.id.connect_button)).setEnabled(false);
    Session.connect(new ConnectionListener() {
      public void onConnected(RealtimeMap session_info) {
        ((Button)findViewById(R.id.connect_button)).setEnabled(true);
        ((Button)findViewById(R.id.connect_button)).setText(R.string.disconnect);

        String msg = "";
        for (Map.Entry<String, Object> entry: session_info.entrySet()) {
          msg += "  * "+entry.getKey()+": "+entry.getValue()+"\n";
        }
        logInfo("Connection state: CONNECTED\n"+msg);

        // ################################################################
        // THIS SAMPLE ENDS HERE. CHECKOUT 'RealtimeRoomManagement' SAMPLE
        // FOR THE NEXT STEPS.
        // ################################################################
      }
      public void onConnectionClosed(int status_code) {
        ...
        logInfo("Connection state: DISCONNECTED\n"
                + "  (connection closed: "+status_code+")");
      }
      public void onConnectionFailed(int status_code) {
        ((Button)findViewById(R.id.connect_button)).setEnabled(true);
        ((Button)findViewById(R.id.connect_button)).setText(R.string.connect);
        logInfo("Connection state: DISCONNECTED\n"
                + "  (connection failed: "+status_code+")");
      }
  ...
  private void disconnectRealtimeSession() {
    ((Button)findViewById(R.id.connect_button)).setEnabled(true);
    ((Button)findViewById(R.id.connect_button)).setText(R.string.connect);
    Session.disconnect();
    logInfo("Connection state: DISCONNECTED");
